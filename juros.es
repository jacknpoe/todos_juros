#!/home/jacknpoe/Executáveis/es/es

# Versão 0.1: 05/01/2026: versão feita sem muito conhecimento de es
...
#          Testes (Debian 12) executaram em cerca de 0.237 segundos por parcela.
...
fn jurosParaAcrescimo {
    ...
    if { !~ `{echo $Quantidade^' < 1 || '^$Periodo^' <= 0.0 || '^$pesoTotal^' <= 0.0 || '^$juros^' <= 0.0' | bc -l } 0 } {
        echo 0.0
        return 1
    }
...
fn acrescimoParaJuros {
    ...
    maxJuros = $4
    pesoTotal = `{getPesoTotal}
    if { !~ `{echo $Quantidade^' < 1 || '^$Periodo^' <= 0.0 || '^$pesoTotal^' <= 0.0 || '^$acrescimo^' <= 0.0 || '^$precisao^' < 1 || '^$maxIteracoes^' < 1 || '^$maxJuros^' <= 0.0' | bc -l } 0 } {
        echo 0.0
        return 1
    }
    ...
    for(interacao = `{seq 1 $maxIteracoes}) {
        if { !~ `{echo $maxJuros^' - '^$minJuros^' < '^$minDiferenca | bc -l } 0 } {
            echo $medJuros
            return 0
        }

        resultado = `{jurosParaAcrescimo $medJuros}
        if { !~ `{echo $resultado^' < '^$acrescimo | bc -l } = 0 } {
            minJuros = $medJuros
        } {
            maxJuros = $medJuros
        }
