...
Versions like those in UCSD Pascal (Apple /// implementation) and Regina Rexx include implementations of the `ln()` function for natural logarithms and `exp()` for Euler’s number raised to a power. Therefore, the `pow()` functions for exponentiation were built using basic mathematical operations. These functions will begin to be ported to languages that do not provide them, such as G-Portugol, Jakt, NPortugol, ugBASIC, lox, and Frege. The B dialect (BCause implementation), in addition to these functions, required the implementation of fixed-point arithmetic (multiplication, division, and conversions).
