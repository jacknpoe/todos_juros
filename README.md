...
Versões como as de UCSD Pascal (Apple /// implementation) e Regina Rexx têm implementações das funções `ln()` para logaritmo natural e `exp()` para número de Euler elevado à potência. Então, as funções `pow()` de exponenciação foram construídas a partir das operações matemáticas básicas. Elas começarão a serem portadas para as linguagens que não têm essas funções, como G-Portugol, Jakt, NPortugol, ugBASIC, lox, Frege, Structured Text (IEC 61131-3 - CODESYS), Suneido. O dialeto B (BCause implementation), além dessas funções, teve aritmética (multiplicação, divisão, conversões) de pontos fixos implementada.
...
Versions like those in UCSD Pascal (Apple /// implementation) and Regina Rexx include implementations of the `ln()` function for natural logarithms and `exp()` for Euler’s number raised to a power. Therefore, the `pow()` functions for exponentiation were built using basic mathematical operations. These functions will begin to be ported to languages that do not provide them, such as G-Portugol, Jakt, NPortugol, ugBASIC, lox, Frege, Structured Text (IEC 61131-3 - CODESYS), and Suneido. The B dialect (BCause implementation), in addition to these functions, required the implementation of fixed-point arithmetic (multiplication, division, and conversions).
