...
The calculation of interest from the increase (`acrescimoParaJuros`) must be done using the `Atingir` line. Enter the desired value in `Juros Simples` (Simple Interest) or `Juros Compostos` (Compound Interest), and click the corresponding button below the value.
