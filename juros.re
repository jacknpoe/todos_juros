...
/** Versão 0.1: 13/08/2024: versão feita sem muito conhecimento de Reason */;
...
let rec rGetPesoTotal = indice =>
  if (indice == 0) {
    pesos[0];
  } else {
    pesos[indice] +. rGetPesoTotal(indice - 1);
  };
...
let rec rJurosCompostos = (indice, juros) =>
  if (indice == 0) {
    ...
    ** (pagamentos[indice] /. periodo)
    +. rJurosCompostos(indice - 1, juros);
  };
...
let rec rAcrescimoParaJuros =
        (acrescimo, minDiferenca, iteracaoAtual, minJuros, maxJuros, medJuros) =>
  if (iteracaoAtual == 0 || maxJuros -. minJuros < minDiferenca) {
    medJuros;
