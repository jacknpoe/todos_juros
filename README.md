...
Dois conceitos que pesaram muito foram a exponenciação de números ponto flutuante (implementada em todas as linguagens) e os arrays dinâmicos (que não foram possíveis em Pascalzim, Portugol e VisuALG, onde os arrays têm mil elementos).
...
Two concepts that weighed heavily were the exponentiation of floating point numbers (implemented in all languages) and dynamic arrays (which were not possible in Pascalzim, Portugol and VisuALG, where arrays have a thousand elements).
