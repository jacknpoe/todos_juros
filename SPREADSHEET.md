...
The calculation of interest from the increase (`acrescimoParaJuros`) should be done using the `Goal Seek` tool. The `Formula cell` must be either the `Acréscimo` (Increase) cell for `Juros Simples` (Simple Interest Rate) (cell $D$19) or the `Acréscimo` (Increase) cell for `Juros Compostos` (Compound Interest Rate) ($E$19). In the `Target value`, enter the desired `Increase` value, but as a percentage (include `%`, as in `10%`). In `Variable cell`, enter the `Juros` (Interest Rate) cell ($D$3).
