...
' Versão 0.1: 16/05/2025: versão feita sem muito conhecimento de OpenOffice Basic (visto que é compatível com LibreOffice Basic em 30/12/2025)
'        0.2: 30/12/2025: funciona para VBA também, a linha com a MsgBox teve os operadores "+" trocados por "&"
...
Function jurosParaAcrescimo(juros)
    ...
    If Quantidade < 1 Or Periodo <= 0# Or pesoTotal <= 0# Or juros <= 0# Then Exit Function
    acumulador = 0#
    For indice = 0 To Quantidade - 1
        If Composto Then
        ...
        End If
    Next indice
    If acumulador <= 0# Then Exit Function
...
Function acrescimoParaJuros(acrescimo, precisao, maxIteracoes, maxJuros)
    ...
    If Quantidade < 1 Or Periodo <= 0# Or pesoTotal <= 0# Or acrescimo <= 0# Or precisao < 1 Or maxIteracoes < 1 Or maxJuros <= 0# Then Exit Function
    minJuros = 0#
    medJuros = maxJuros / 2#
