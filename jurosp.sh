...
jurosParaAcrescimo() {
    ...
    errado=$(echo "$Quantidade < 1 || $Periodo <= 0.0 || $pesoTotal <= 0.0 || $juros <= 0.0" | bc -l)
    if [ "$errado" = "1" ] ; then
        echo "0.0"
        return 1
    fi
...
acrescimoParaJuros() {
    ...
    maxJuros=$4
    pesoTotal=$(getPesoTotal)
    errado=$(echo "$Quantidade < 1 || $Periodo <= 0.0 || $pesoTotal <= 0.0 || $acrescimo <= 0.0 || $precisao < 1 || $maxIteracoes < 1 || $maxJuros <= 0.0" | bc -l) 
    if [ "$errado" = "1" ] ; then
        echo "0.0"
        return 1
    fi
