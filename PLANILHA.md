...
A planilha [juros.ods](juros.ods) (**Cálculo de Acréscimos e Juros**) implementa os algoritmos `getPesoTotal`, `jurosParaAcrescimo` e `acrescimoParaJuros` utilizados neste projeto. Seu objetivo é servir como referência para validação das implementações nos diversos dialetos de programação e também como ferramenta para experimentação com diferentes **períodos**, **pagamentos**, **pesos** e **taxas de juros**. A planilha está limitada a até `12` `Parcelas`, embora com alterações possa ser escalada mantendo a mesma lógica.

O `Peso Total` é simplesmente dos valores da soma da coluna `Peso`.
...
O cálculo dos juros a partir do acréscimo (`acrescimoParaJuros`) deve ser feito usando a linha `Atingir`. Coloque o valor desejado em em `Juros Simples` ou `Juros Compostos`, e clique no botão correspondente, abaixo do valor.
